...
data class AdbLogcatDefaultFormatParser(
        ...
        override fun create(sample: String): LogParser? {
            try {
                ...
                val includeDate = try {
                    ...
                    val yearAdded = "${LocalDate.now().year}-$currentToken"
                    LocalDate.parse(yearAdded)
                    currentToken = iterator.next()
                    true
                } catch (e: Exception) {
                    false
                ...
                if (currentToken.isPriority()) {
                    return AdbLogcatDefaultFormatParser(
                        includeDate,
                        includeTime,
                        includePid,
                        includeTid,
